...
const renderMobileMenuItem = (item: MenuItem) => {
  if (item.items) {
    return (
      <AccordionItem key={item.title} value={item.title} className="border-b-0">
        <AccordionTrigger className="text-md py-0 font-bold hover:no-underline">
          {item.title}
        </AccordionTrigger>
  ...
  return (
    <a key={item.title} href={item.url} className="text-md font-bold">
      {item.title}
    </a>
  );
};
...
const SubMenuLink = ({ item }: { item: MenuItem }) => {
  return (
    <a
      className="flex min-w-80 flex-row gap-4 rounded-md p-3 leading-none no-underline transition-colors duration-200 outline-none select-none hover:bg-accent/10 hover:text-accent"
      href={item.url}
    >
